...
!!! note "Status — Stable (Gaussian mean)"
    Mirrors drmTMB's [Phylogenetic structured effects](https://itchyshin.github.io/drmTMB/articles/phylogenetic-models.html).
    **In DRM.jl today:** `phylo(1 | species)` on the Gaussian **mean** — a
    phylogenetic random intercept fit in closed form from a tree. The q=4
    phylogenetic *location-scale* model (effect on `log σ` too) uses the verified
    sparse-Laplace engine (see `HANDOVER.md`).
