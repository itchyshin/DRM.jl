...
| bridge names = base-R (measurement) + stale fixtures re-keyed | #584 (draft) | DRM.jl's own rendering already matches base R on all ten design-258 constructs (20/20); the six `bridge-*` fixtures were keyed by pre-`62c4e6a2` names → re-keyed, 1/7 → 7/7 under `DRM_PARITY_TESTS=1`. **Design 258 §2 row 7 is wrong** (nine-name full-dummy list; base R gives six, reduced coding) — reported to the drmTMB lane |
| label echo (`coef_labels` → `bridge_formula_labels_v1` verbatim, fail-closed on count) | next | nothing in `src/bridge.jl` reads `coef_labels` (0 hits); waiting on the drmTMB lane for the exact wire field of `af1790492`; approved PR-gated (D-203 §3b) |
