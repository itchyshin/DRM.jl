...
| S6 family/formula/bridge gaps | **merged** as #596 (`f96f1bb7`; closes #546) | recon found the bridge already routes `sd()`/`sd_phylo()` (`src/bridge.jl:683`) and unknown keys throw (`:630`); the gap was proof: same-target matrix `test/test_bridge_lss_routes.jl`, 6 route cells + #546 guard, 47/47, bridge vs direct bit-identical; closes the Julia half of #546; after-task reports for #590/#593/#594 and for S6 |

| Documenter red on `main` after #589/#590 (docstrings without `@docs`; five merges cancelled each other's `main` run) | **merged** as #598 (`bd1c834e`) | two `@docs` entries + `pack_phi` de-linked; `Documenter.yml` cancel-in-progress scoped to PR refs so `main` runs always complete; process lesson recorded (LESSONS): never merge a PR whose current-head checks have not completed |
| S7(a) codex leaf `lss-tip-identity` G8 (strict boundary gate) | **merged** as #599 (`542e5321`) | diagnosis first: both fits are stationary optima of the same objective (Δloglik 7e-15) on a Hessian-singular ridge in the 2-parameter `sd_phylo` block → raw-θ 4e-6 was an unfulfillable oracle; gate now compares per-tip σ_phy (1.2e-8 vs 4e-6) + loglik; strict and default 410/410; the default suite's 1 Broken gone honestly |
| S7(b) sparse multi-component LSS routing/derivatives | not started | dense multi-component (`_fit_gaussian_lss_multi`, exact gradients) is landed; sparse multi-component does not exist — new engine work: design note (symbolic alignment) before code; contract 6 forbids a universal O(p) claim for crossed structures |

| #577 ML-path structural zeros (`prior_precision` root fix, separate Claude lane shinichi-ce) | **merged** as #597 (`77513aa0`; closes #577) | full suite green on the pre-merge head; unblocks two follow-ups owned here: collapse the now-redundant `_reml_prior_precision` (bit-identical P, delete, re-run #575 tests) and re-measure `gaussian_phylo_mean` / `phylo_count_large_p` on the fixed ML path |

| #577 follow-ups | **merged** #601 (`b880f19e`, ML-phylo rows re-measured: within tolerance, movements 2–5 orders below tol) and #602 (`92fe3e36`, `_reml_prior_precision` collapsed after a bit-identical proof, nnz 1376) | both merged only after their full check sets completed on the current head |
| S7(b) sparse multi-component LSS — Phase 1 design | #603 (open; docs) | fill measured: nested / one-phylo O(p) (nnz(L)/p ≈ 3.0 to p = 4000), two crossed factors near-dense (nnz(L)/dim² ≈ 0.35) → GO nested class / NO-GO general crossed (vault **D-206**); adversarial review caught the diagonal-only α-gradient error (91% vs 3.4e-10) before code; Phase 2 (20–28 agent-hours, checks on Totoro per D-205) approved as a fresh task |
| drmTMB side landed (drmTMB lane, Shinichi's word) | drmTMB #1114 → `37ea93c47`, #1119 → `8fda9b017` (= drmTMB main) | drmTMB main sends `options["coef_labels"]` on every bridge fit (echo live end to end, incl. `phylocov` / `sd` blocks, design 258 §7.5 — refs may now cite drmTMB `origin/main`); A4/A5 pinned to DRM.jl `77513aa0` via `drm_bridge_objective_at`; promotion wave 1: four rows `partial` on the bridge axis, `claim_status` unchanged, `parity_ledger.py` CLOSURE PASS; `lss-tip-identity` G6 receipt regenerated at 77513aa0 |
