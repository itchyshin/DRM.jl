...
## Findings — corrected by `CORRECTION-20260831.md`
...
2. **P1 — unresolved mobile-light rendering/capture failure.** `07-location-scale-mobile-light.png` shows an unusable narrow right-column layout; `08-location-scale-mobile-light-full.png` also has capture corruption. The earlier no-blocker verdict is withdrawn. `CORRECTION-20260831.md` gives the exact sequence, contrary evidence, and required clean recheck. The raw build's missing `versions.js` / `siteinfo.js` resources remain a separate known metadata failure. This sample cannot establish G6, an all-page visual audit, a fresh candidate build, deployed-site correctness, keyboard traversal, or an assistive-technology audit.
