...
log="$run_dir/profile-nuisance-corrected-replay-${stamp}.log"
before="$run_dir/profile-nuisance-corrected-replay-${stamp}.before.sha256"
after="$run_dir/profile-nuisance-corrected-replay-${stamp}.after.sha256"
command_receipt="$run_dir/profile-nuisance-corrected-replay-${stamp}.command.txt"
pid_receipt="$run_dir/profile-nuisance-corrected-replay-current.pid"
artifact="$run_dir/profile-nuisance-corrected-replay-${stamp}.jls"
script="$run_dir/replay-profile-nuisance-termination-corrected.jl"
...
with open(log, "w", encoding="utf-8") as stream:
    ...
    print(f"S11_CORRECTED_REPLAY_WALL_SECONDS={time.monotonic() - started:.6f}", file=stream)
sys.exit(status)
PY
status=$?
set -e
