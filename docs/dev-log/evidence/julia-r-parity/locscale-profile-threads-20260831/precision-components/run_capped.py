#!/usr/bin/env python3
# Foreground fresh-process runner. Usage: UTCSTAMP; hard cap 20 seconds.
...
outdir = Path('/private/tmp/drm-parity-20260830/profile-threads-s11/precision-components')
script = outdir / 'diagnose.jl'
log = outdir / f'precision-components-{stamp}.log'
status = outdir / f'precision-components-{stamp}.status.json'
...
payload = {'kind':'fixed_precision_components_capped','mode':mode,'stamp':stamp,'command':cmd,
           ...
           'hard_cap_seconds':20,'timed_out':timed_out,'returncode':rc,'log':str(log),'script':str(script)}
