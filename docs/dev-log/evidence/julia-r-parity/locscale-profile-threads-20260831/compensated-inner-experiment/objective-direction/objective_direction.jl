...
load_candidate_definitions() # Top-level import before compiling callers.
install_compensated_override!() # Install at top level; never run the old experiment.
...
function main()
    ...
    @assert S11_CANDIDATE_CALLS[] == 4 && S11_FALLBACK_CALLS[] == 0
    after = hashes()
    @assert before == after
