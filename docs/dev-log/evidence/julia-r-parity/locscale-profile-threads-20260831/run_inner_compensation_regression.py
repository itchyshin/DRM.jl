...
log = base / f'inner-compensation-regression-{label}-{stamp}.log'
paths = [repo / name for name in (
    'src/locscale_grad.jl', 'src/locscale_inner.jl', 'src/inference.jl',
    'test/test_locscale_compensated_gradient.jl',
...
command = ['julia', '--startup-file=no', f'--project={repo}', '-e',
           'include("test/test_locscale_compensated_gradient.jl"); println("LOCSCALE_COMPENSATED_GRADIENT_OK")']
