...
function exact_design(r)
    ...
    Q = Matrix{Float64}(I, dat.G, dat.G)
...
function production_components(design, theta)
    ...
    Pdirect = kron(design.Q, direct_precision(lambda))
...
function production_start_result(design, parts, theta, label, astart)
    ...
    packed = DRM._ls_fit_nll(Val(:gamma), design.y, design.Xmu, design.Xpsi,
    ...
    P64a64 = parts.P * a
    lifted_same_float = BigFloat.(parts.P) * BigFloat.(a) + BigFloat.(data64)
    ...
    return (label=label, start=copy(astart), a=copy(a), inner_ok=inner_ok,
            ...
            packed_fit_nll=packed, packed_matches_raw=(marginal_ok ? packed == raw : packed == 1e18),
