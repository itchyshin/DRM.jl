...
stamp=time.strftime('%Y%m%dT%H%M%SZ',time.gmtime());out=base/('neighbours-'+stamp)
script=base/'neighbours.jl'; snap=out.with_suffix('.script.jl');snap.write_bytes(script.read_bytes())
paths=list((repo/'src').glob('*.jl'))+[repo/'test'/name for name in ['test_locscale_profile_threads.jl','test_locscale_frontend.jl','test_locscale_infer.jl','test_sigma_axis_re.jl']]
