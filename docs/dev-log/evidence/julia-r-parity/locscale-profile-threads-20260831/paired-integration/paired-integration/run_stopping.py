...
stamp=time.strftime('%Y%m%dT%H%M%SZ',time.gmtime());out=base/('stopping-'+stamp)
script=base/'polish_stopping_probe.jl'; snap=out.with_suffix('.script.jl');snap.write_bytes(script.read_bytes())
...
r=dict(command=cmd,seconds=time.monotonic()-start,estimate_seconds=10,cap_seconds=30,code=code,timed_out=timeout,before=before,after=after,unchanged=before==after,script_sha256=sha(snap))
