...
function inside_strict_clamp(z, eta0, psi0, gidx, B)
    a = B * z
    for i in eachindex(eta0)
        g = gidx[i]
        abs(eta0[i] + a[2g-1]) < CLAMP || return false
        abs(psi0[i] + a[2g]) < CLAMP || return false
    end
    return true
end

function damped_descent_direction(H, g)
    T = eltype(g)
    undamped = cholesky(Symmetric(H); check=false)
    if isposdef(undamped)
        d = -(undamped \ g)
        dot(g, d) < zero(T) && return (direction=d, damping=zero(T), undamped_pd=true)
    end
    scale = max(one(T), maxabs(diag(H)))
    damping = scale * big"1e-20"
    for _ in 1:100
        damped = cholesky(Symmetric(H + damping * Matrix{T}(I, size(H)...)); check=false)
        if isposdef(damped)
            d = -(damped \ g)
            dot(g, d) < zero(T) && return (direction=d, damping=damping, undamped_pd=isposdef(undamped))
        end
        damping *= 2
    end
    error("no PD descent direction after bounded diagonal damping")
end
...
function solve_mode(zstart, y, eta0, psi0, gidx, B, tol)
    ...
    for iter in 0:100
        ...
        undamped = cholesky(Symmetric(state.H); check=false)
        if residual <= tol
            isposdef(undamped) || error("residual gate reached at non-PD final Hz; trace=$(repr(trace))")
            push!(trace, (iter=iter, J=state.J, residual=residual, undamped_pd=true,
                          damping=zero(eltype(z)), alpha=zero(eltype(z)), accepted=true,
                          roundoff_acceptance=false, reason=:final))
            return (z=z, state=state, iterations=iter, residual=residual, trace=trace)
        end
        proposal = damped_descent_direction(state.H, state.g)
        direction = proposal.direction
        directional_derivative = dot(state.g, direction)
        @assert directional_derivative < zero(eltype(z)) "globalized solver lost descent"
        alpha = one(eltype(z))
        accepted = false
        for backtrack in 0:60
            trial_z = z + alpha * direction
            if inside_strict_clamp(trial_z, eta0, psi0, gidx, B)
                trial = oracle_terms(trial_z, y, eta0, psi0, gidx, B)
                armijo = trial.J <= state.J + armijo_c1 * alpha * directional_derivative
                trial_residual = maxabs(trial.g)
                roundoff = proposal.damping == zero(eltype(z)) && alpha == one(eltype(z)) &&
                           maxabs(direction) <= sqrt(eps(state.J)) &&
                           abs(trial.J - state.J) <= 4 * eps(state.J) && trial_residual < residual
                if armijo || roundoff
                    push!(trace, (iter=iter, J=state.J, residual=residual,
                                  undamped_pd=proposal.undamped_pd, damping=proposal.damping,
                                  alpha=alpha, accepted=true, roundoff_acceptance=roundoff,
                                  reason=roundoff ? :bounded_roundoff : :armijo))
                    z = trial_z
                    accepted = true
                    break
                end
        ...
        accepted || error("bounded Armijo backtracking failed; trace=$(repr(trace)), iter=$iter, damping=$(proposal.damping), final_alpha=$alpha")
    end
    error("mode residual gate not reached in 100 Newton steps; trace=$(repr(trace))")
end
...
function evaluate_case(label, theta64, state, bits; transformed_a=nothing)
    setprecision(BigFloat, bits) do
        ...
        return (label=label, bits=bits, lambda=lambda, L=L, M=M, J=base.J,
                ...
                trace_zero=solved_zero.trace, trace_transformed=solved_transformed.trace,
