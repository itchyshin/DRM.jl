...
function trigamma_big(x)
    ...
    central(step) = (digamma(x + step) - digamma(x - step)) / (2step)
    richardson(step) = (4central(step / 2) - central(step)) / 3
    value = richardson(h)
...
function numeric_derivative_control(bits)
    setprecision(BigFloat, bits) do
        ...
        d1(f1, h) = (-f1(2h) + 8f1(h) - 8f1(-h) + f1(-2h)) / (12h)
        d2(f1, h) = (-f1(2h) + 16f1(h) - 30f1(0) + 16f1(-h) - f1(-2h)) / (12h^2)
        ...
        mixed(h) = (f(eta + h, psi + h) - f(eta + h, psi - h) -
                    f(eta - h, psi + h) + f(eta - h, psi - h)) / (4h^2)
        nhep = (4mixed(h2 / 2) - mixed(h2)) / 3
        analytic = BigFloat[ge, gp, hee, hep, hpp]
        numeric = BigFloat[nge, ngp, nhee, nhep, nhpp]
        err = maxabs(analytic .- numeric)
        gate = bits == 128 ? big"1e-18" : big"1e-30"
        @assert err <= gate "independent Gamma derivative control failed"
        return (analytic=analytic, numeric=numeric, maxabs_error=err, gate=gate,
...
for bits in (128, 256)
    ...
    println("S11_WHITE_DERIVATIVE_CONTROL=", repr((bits=bits, control=numeric_derivative_control(bits))))
