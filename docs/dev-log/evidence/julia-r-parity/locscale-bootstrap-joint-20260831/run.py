...
name=sys.argv[1];names={'neighbours':['test_simulate.jl','test_simulate_scale_conventions.jl'],'scales':['test_simulate_scale_conventions.jl'],'sampler':['test_locscale_bootstrap_simulator.jl']}[name]
stamp=time.strftime('%Y%m%dT%H%M%SZ',time.gmtime());out=b/(name+'-'+stamp)
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
paths=list((r/'src').glob('*.jl'))+[r/'test'/n for n in names]
before={str(p.relative_to(r)):sha(p) for p in paths}
script='using DRM,Test,LinearAlgebra; @assert Threads.nthreads()==1 && BLAS.get_num_threads()==1\n@testset "bounded bootstrap regression" begin\n'+''.join('include('+json.dumps(str(r/'test'/n))+')\n' for n in names)+'end\nprintln("BOOTSTRAP_SLICE_PASS")\n'
...
with out.with_suffix('.log').open('xb') as f:
 p=subprocess.Popen(cmd,cwd=r,env=dict(os.environ,JULIA_NUM_THREADS='1',OPENBLAS_NUM_THREADS='1'),stdout=f,stderr=subprocess.STDOUT,start_new_session=True)
...
rj=dict(command=cmd,code=code,seconds=time.monotonic()-start,estimate_seconds=30,cap_seconds=30,timed_out=timeout,before=before,after=after,unchanged=before==after,julia_threads=1,blas_threads=1)
