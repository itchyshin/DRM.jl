...
name=sys.argv[1];names={'neighbours':['test_simulate.jl','test_simulate_scale_conventions.jl'],'scales':['test_simulate_scale_conventions.jl'],'sampler':['test_locscale_bootstrap_simulator.jl'],'diagnostic':[]}[name]
...
script=(b/'refit-diagnostic.jl').read_text()
...
rj=dict(command=cmd,code=code,seconds=time.monotonic()-start,estimate_seconds=30,cap_seconds=30,timed_out=timeout,before=before,after=after,unchanged=before==after,julia_threads=nt,blas_threads=1)
