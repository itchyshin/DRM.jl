...
for (nm in names(forms)) {
  receipt$cases[[nm]] <- tryCatch({
    ...
    for (mode in c('default', 'tight_diagnostic')) {
      ...
      if (mode == 'default') {
        # Same start and controls, two independent objective implementations.
        # Preserve these separate diagnostic solves; neither replaces a fit.
        refinement <- lapply(c('analytic', 'native_TMB'), function(provider) {
          fn <- if (provider == 'analytic') objective else fit$obj$fn
          gr <- if (provider == 'analytic') gradient else fit$obj$gr
          opt <- nlminb(theta, fn, gr, control = tight)
          list(provider = provider, convergence = opt$convergence, message = opt$message,
            iterations = opt$iterations, coefficients = unname(opt$par), objective = objective(opt$par),
            objective_improvement = objective(theta) - objective(opt$par),
            gradient = gradient(opt$par), max_abs_gradient = max(abs(gradient(opt$par))),
            max_abs_coefficient_difference_from_frozen_Julia = max(abs(opt$par - recovered)))
        })
      }
    }
    ...
    list(status = if (reproduction && all(vapply(results, check, logical(1)))) 'PASS' else 'FAIL',
      default_reproduced = reproduction, results = results, refinement = refinement,
