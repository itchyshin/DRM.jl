...
if (length(args) != 3L) stop('usage: joint_native_uncertainty_probe.R FROZEN_JSON R_LIB NEW_JSON [--preflight]')
...
out$status <- if(all(vapply(out$cases,function(z) identical(z$status,'PASS'),logical(1)))) 'PASS' else 'FAIL'
