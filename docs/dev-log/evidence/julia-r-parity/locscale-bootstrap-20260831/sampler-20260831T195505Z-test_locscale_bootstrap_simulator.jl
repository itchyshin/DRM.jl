...
function _locscale_bootstrap_public_fixture()
    ...
    d = sqrt.(diag(Craw))
    K = Craw ./ (d * d')
    Q, gidx, Gfound = DRM._locscale_relmat_setup(K, g)
    @assert Gfound == G
    ...
    formula = bf(@formula(y ~ x + (1 | bootstrap_pair | relmat(g))),
...
@testset "canonical location-scale marginal bootstrap simulator" begin
    ...
    simulator = DRM._marginal_simulator(fx.fit, fx.data; K = fx.K)
