...
threaded <- identical(Sys.getenv('PROVIDER_THREADS', 'false'), 'true')
set.seed(20260831)
G <- 8L
m <- 5L
pos <- matrix(runif(G * 2L, 0, 3), G, 2L)
D <- as.matrix(dist(pos))
K <- exp(-D / 0.8) + diag(1e-8, G)
id <- rep(sprintf('g%02d', seq_len(G)), each = m)
x <- rnorm(G * m)
u <- 0.35 * drop(t(chol(K)) %*% rnorm(G))
lambda <- exp(0.25 + 0.30 * x + u[match(id, unique(id))])
y <- stats::rpois(G * m, lambda)
...
stopifnot(
  identical(fit$bridge_payload$kwarg, 'K'),
  identical(dim(fit$bridge_payload$matrix), dim(K)),
  isTRUE(all.equal(
    unname(fit$bridge_payload$matrix), unname(K), tolerance = 0
  ))
)
...
JuliaCall::julia_command(paste(
  'drmTMB_direct_poisson_provider(formula,family,data,K,options,threads)=',
  'DRM.drm_bridge_inference(formula=formula,family=family,data=data,K=K,',
  'options=options,method="bootstrap",level=.95,B=2,seed=7101,',
  'threads=threads,parm="fixef:mu:x")'
))
direct <- JuliaCall::julia_call(
  'drmTMB_direct_poisson_provider',
  fit$bridge_payload$formula,
  fit$model$model_type,
  as.list(fit$bridge_payload$data),
  K,
  if (length(fit$bridge_payload$options)) fit$bridge_payload$options else NULL,
  threaded
)
JuliaCall::julia_command(paste(
  'drmTMB_direct_poisson_profile(formula,family,data,K,options)=',
  'DRM.drm_bridge_inference(formula=formula,family=family,data=data,K=K,',
  'options=options,method="profile",level=.95,threads=false,',
  'parm="fixef:mu:x")'
))
direct_profile <- JuliaCall::julia_call(
  'drmTMB_direct_poisson_profile',
  fit$bridge_payload$formula,
  fit$model$model_type,
  as.list(fit$bridge_payload$data),
  K,
...
result <- list(
  ...
  threaded = threaded,
  julia_threads = as.integer(JuliaCall::julia_eval('Threads.nthreads()')),
  blas_threads = as.integer(JuliaCall::julia_eval(
    'DRM.LinearAlgebra.BLAS.get_num_threads()'
  )),
  payload_kwarg = fit$bridge_payload$kwarg,
  formula = fit$bridge_payload$formula,
  observed = observed,
  expected = expected,
...
stopifnot(
  ...
  identical(ci$bootstrap.n[[1L]], 2L),
  identical(ci$bootstrap.failed[[1L]], 0L),
  identical(ci$conf.status[[1L]], 'bootstrap'),
  identical(ci_profile$conf.status[[1L]], 'profile'),
  identical(result$blas_threads, 1L)
)

saveRDS(result, Sys.getenv('PROVIDER_RESULT_RDS'))
cat('ACTUAL_R_POISSON_RELMAT_BOOTSTRAP_PROFILE_PASS\n')
