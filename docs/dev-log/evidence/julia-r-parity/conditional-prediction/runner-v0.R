...
receipt <- list(scope = 'ML Gaussian one ordinary mean random intercept, stored conditional and newdata zeroRE prediction',
  ...
  independent_fit_tolerance = 4e-6, adapter_tolerance = 1e-10, expected_predictions = 16L,
  R_checkout = pkg, Julia_checkout = jl, R_bridge_sha256 = sha(file.path(pkg, 'R/julia-bridge.R')),
...
if (!receipt$bridge_unchanged_during_run) receipt$status <- 'FAIL'
save_receipt()
