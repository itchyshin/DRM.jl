...
probe = out / 'verify_tracked_saved_oracles.jl'
...
def hashes():
    paths = sorted(repo.glob('src/**/*.jl')) + [probe, repo/'Project.toml', repo/'Manifest.toml', Path('/private/tmp/locscale_tracked_prior_six_state.jl'), Path('/private/tmp/locscale_objective_arithmetic_states-001.jls'), Path('/private/tmp/locscale_gamma_vcov_actual_context-001.jls')]
...
receipt = dict(started_utc=started, elapsed_seconds=time.monotonic()-clock,
               exit_code=rc, timed_out=timeout, cap_seconds=45,
