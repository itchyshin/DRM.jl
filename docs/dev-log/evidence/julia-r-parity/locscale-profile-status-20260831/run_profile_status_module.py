...
out=repo/'docs/dev-log/evidence/julia-r-parity/locscale-profile-status-20260831'
threads=int(sys.argv[1]); label='module-'+str(threads)+'threads-'+(sys.argv[2] if len(sys.argv)>2 else '001')
files=['test/test_locscale_profile_status.jl','test/test_bridge_profile_status.jl','test/test_inference_blas_pinning.jl','test/test_profile_nuisance_status.jl']
def hashes():
