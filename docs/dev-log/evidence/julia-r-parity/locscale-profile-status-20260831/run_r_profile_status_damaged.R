...
orig_sd <- getFromNamespace('drm_julia_inference_confint_row', 'drmTMB')
orig_fixef <- getFromNamespace('drm_julia_fixef_inference_confint_row', 'drmTMB')
testthat::local_mocked_bindings(
  drm_julia_inference_confint_row = function(...) {
    out <- orig_sd(...); out$conf.status <- 'profile'; out$profile.message <- 'lost'; out
  },
  drm_julia_fixef_inference_confint_row = function(...) {
    out <- orig_fixef(...); out$conf.status <- 'profile'; out$profile.message <- 'lost'; out
  }, .package='drmTMB', .env=globalenv()
)
