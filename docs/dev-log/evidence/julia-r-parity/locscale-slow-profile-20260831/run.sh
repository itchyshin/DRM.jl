...
out=locscale-slow-profile-pilot-001
export DRM_SLOW_TESTS=1 JULIA_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 JULIA_PKG_OFFLINE=true JULIA_PKG_PRECOMPILE_AUTO=0
start=$(date +%s)
date -u +%FT%TZ
...
timeout -k 10 900 /home/snakagaw/.juliaup/bin/julia +1.10.10 --project=test --startup-file=no "$out/run.jl"
rc=$?
