...
nt=int(sys.argv[1]); assert nt in (1,4)
name=sys.argv[2] if len(sys.argv)>2 else 'tree'
names={'tree':['test_bridge_bootstrap_tree.jl'], 'neighbours':['test_bridge_formula_labels.jl','test_bridge_lss_labels.jl','test_bridge_profile_status.jl','test_locscale_bootstrap_refit.jl']}[name]
...
with out.with_suffix('.log').open('xb') as f:
 p=subprocess.Popen(cmd,cwd=r,env=dict(os.environ,JULIA_NUM_THREADS=str(nt),OPENBLAS_NUM_THREADS='1',DRM_TREE_BOOTSTRAP_FIXTURE_DIR=str(b/'fixture')),stdout=f,stderr=subprocess.STDOUT,start_new_session=True)
...
if code == 0 and before == after: print('TREE_BOOTSTRAP_CHECK_PASS')
