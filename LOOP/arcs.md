...
| **A2a** | pending | result-shape contract (Julia) | Enrich `drm_bridge` / `drm_bridge_inference` payload so drmTMB's R post-fit works unchanged on a Julia fit: `fitted_distribution`, `qq_plot`, `worm_plot`, `centile_chart`, `exceedance`, `predict(dpar="sigma")`, scale/variance Wald blocks. `julia-engine.Rmd` names the shortfall | 1–2 d | no |
