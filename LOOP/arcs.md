...
| S3 | TDD red: standalone `test/test_cox_reid_poisson_phylo.jl` (not in `runtests.jl`) | todo | — |
| S4 | Wire: lift structured `_reject_reml_route`; thread `reml` into `_fit_poisson_general_laplace`; reuse #444 helpers | todo | — |
| S5 | Docs: Poisson docstring warning (Cell D not recovery; ML default; no chip) | todo | — |
| S6 | `Pkg.test()`; read logs; no recovery sentence | todo | — |
