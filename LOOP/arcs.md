...
| **A3a** | pending | `biv_lognormal` | **RE-SCOPED — next arc.** Closed-form (dr18): bivariate normal on `log y` × Jacobian `∏1/yᵢ`. Reuses `_fit_bivariate_residual` (`gaussian_bivariate.jl:187`) + log-transform + `+Σ log yᵢ` + `y>0` check. Constant `sigma1/sigma2/rho12`, matching drmTMB's boundary | 0.5–1 d | no |
