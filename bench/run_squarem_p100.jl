# run_squarem_p100.jl — SQUAREM-EM vs drmTMB on real q4_p100 (drmTMB 2.48s, LL -256.52).
# Compares against the warm-noSQS baseline (9.76s, LL -259.58, 60 iters not converged).
using LinearAlgebra, SparseArrays, Random, Statistics, Printf, CSV, DataFrames
include(joinpath(@__DIR__, "fit_em_squarem.jl"))
...
println("=== SQUAREM-EM, real q4_p100 (p=$p) ===")
fit_em_squarem(prob,Q_cond,β0,Matrix(0.3*I(4));max_em=2,verbose=false)  # warmup
t=@elapsed r=fit_em_squarem(prob,Q_cond,β0,Matrix(0.3*I(4));max_em=60,tol=1e-4,verbose=false)
println("\n--- result ---")
@printf "logLik Julia=%.4f  drmTMB=%.4f  |Δ|=%.4f\n" r.loglik DRMTMB_LL abs(r.loglik-DRMTMB_LL)
@printf "wall Julia=%.2fs  drmTMB=%.2fs  ratio=%.2fx  (warm-noSQS was 9.76s)\n" t DRMTMB_T (DRMTMB_T/t)
@printf "iters=%d (warm-noSQS=60 not converged)   Λ diag=%s\n" r.iters round.(diag(r.Λ);digits=3)
println("=== done ===")
