# fit_phylo_poisson.jl — Julia side of the Poisson phylo benchmark.
#
# Run from repo root after `Rscript bench/R/gen_phylo_poisson.R`:
...
const FIXDIR = joinpath(@__DIR__, "fixtures", "phylo_poisson")
const OUTDIR = joinpath(@__DIR__, "results", "phylo_poisson")
mkpath(OUTDIR)
...
function fit_cell(cell)
    ...
    drm(bf(@formula(y ~ x + phylo(1 | species))), Poisson();
        data = (; y, x, species), tree = tree, g_tol = 1e-7, se = false)
end
...
function run_cell(cell)
    ...
    @printf("[Julia %s] p=%d n=%d med=%.4fs logLik=%.3f beta=(%.3f, %.3f) sd=%.3f\n",
            cell.id, cell.p, cell.n, row[6][2], row[8][2],
            row[10][2][1], row[10][2][2], row[11][2])
    return row
end

results = [run_cell(cell) for cell in CELLS]
open(joinpath(OUTDIR, "julia_phylo_poisson.json"), "w") do io
    println(io, "[")
