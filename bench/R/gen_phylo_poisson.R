## gen_phylo_poisson.R — deterministic Poisson phylo fixtures.
##
## Run from repo root:
...
cells <- list(
  list(id = "phylo_p100", p = 100, m = 5, n = 500, reps = 5, seed = 2606101),
  list(id = "phylo_p500", p = 500, m = 3, n = 1500, reps = 3, seed = 2606102),
  list(id = "phylo_p1000", p = 1000, m = 2, n = 2000, reps = 3, seed = 2606103),
  list(id = "phylo_p2000", p = 2000, m = 2, n = 4000, reps = 2, seed = 2606104)
)
...
simulate_cell <- function(cell) {
  ...
  y <- stats::rpois(length(species), lambda = exp(eta))
  ...
  cat(sprintf("[fixture %s] p=%d n=%d mean_y=%.3f\n",
              cell$id, cell$p, length(species), mean(y)))
}
