...
# (DRM_TEST_SHARD="k/N"). No package `using` needed — plain Base only.
...
function _parse_shard_spec(spec::AbstractString)
    parts = split(spec, '/')
    length(parts) == 2 ||
        throw(ArgumentError("malformed DRM_TEST_SHARD spec $(repr(spec)); expected \"k/N\""))
    k = tryparse(Int, parts[1])
    N = tryparse(Int, parts[2])
    (k === nothing || N === nothing) &&
        throw(ArgumentError("malformed DRM_TEST_SHARD spec $(repr(spec)); k and N must be integers"))
    N >= 1 ||
        throw(ArgumentError("DRM_TEST_SHARD spec $(repr(spec)): N must be >= 1, got N=$N"))
    1 <= k <= N ||
        throw(ArgumentError("DRM_TEST_SHARD spec $(repr(spec)): k must satisfy 1 <= k <= N, got k=$k, N=$N"))
    return k, N
end
