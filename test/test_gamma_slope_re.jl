# Gamma correlated random intercept+slope (1 + x | g): a positive-continuous GLMM,
...
@testset "Gamma correlated random slope (1+x|g) — recovery" begin
    ...
    fit = drm(bf(@formula(y ~ x + (1 + x | g)), @formula(sigma ~ 1)), Gamma(); data = (; y, x, g))
